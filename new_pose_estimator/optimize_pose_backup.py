"""
Clean, elegant 3-DOF pose refinement using silhouette matching.
Optimizes only rotation while keeping translation fixed.
...
- 3-DOF optimization (rotation only)
"""
...
class PoseOptimizer:
    """Pose optimization using multiple strategies (rotation only)."""
    ...
    def _pose_to_params(self, rvec: np.ndarray, tvec: np.ndarray) -> np.ndarray:
        """Convert pose to optimization parameters (rotation only)."""
        return rvec.copy()
    
    def _params_to_pose(self, params: np.ndarray, fixed_tvec: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """Convert optimization parameters to pose (rotation only)."""
        return params.copy(), np.array(fixed_tvec, dtype=np.float64)
    ...
    def _objective_function(self, params: np.ndarray, target_mask: np.ndarray, 
                           fixed_tvec: np.ndarray, metric: str = 'iou') -> float:
        """Objective function for optimization."""
        try:
            rvec, tvec = self._params_to_pose(params, fixed_tvec)
    ...
    def optimize(self, target_mask: np.ndarray, init_rvec: np.ndarray, init_tvec: np.ndarray,
        ...
        """
        Optimize pose to match target silhouette (rotation only).
        
        Args:
            target_mask: Binary target silhouette
            init_rvec: Initial rotation vector
            init_tvec: Fixed translation vector (will not be optimized)
        ...
        """
        logger.info(f"Starting 3-DOF pose optimization with {method} method, {metric} metric")
        
        init_params = self._pose_to_params(init_rvec, init_tvec)
        init_loss = self._objective_function(init_params, target_mask, init_tvec, metric)
        
        logger.info(f"Initial loss: {init_loss:.4f}")
        logger.info(f"Fixed translation: {init_tvec}")
        
        if method == 'differential_evolution':
            # Global optimization with rotation bounds only
            bounds = [
                (-1, 1),  # quaternion x component
                (-1, 1),  # quaternion y component  
                (-1, 1)   # quaternion z component
            ]
            
            result = differential_evolution(
                self._objective_function, bounds,
                args=(target_mask, init_tvec, metric),
        ...
        final_rvec, final_tvec = self._params_to_pose(result.x, init_tvec)
        ...
        return {
            ...
            'rvec': final_rvec,
            'tvec': final_tvec,  # Same as input (fixed)
            'loss': final_loss,
            'init_loss': init_loss,
...
class PoseRefiner:
    ...
    def refine_pose(self, target_mask: np.ndarray, init_rvec: np.ndarray, init_tvec: np.ndarray,
        ...
        """
        Refine pose using the specified strategy (rotation only).
        ...
        if strategy == 'single':
            ...
            result2 = self.optimizer.optimize(
                target_mask, result1['rvec'], init_tvec,  # Keep original tvec
    ...
    def save_img(self, path, img):
        path = Path(path)
        for i in range(100):
            p = path if i == 0 else path.with_name(f"{path.stem}{i}{path.suffix}")
            if not p.exists():
                cv2.imwrite(str(p), img)
                break

    def visualize_results(self, image, target_mask: np.ndarray, 
        ...
        # Save outputs
        self.save_img(str(output_path / 'target_mask.png'), target_mask)
        self.save_img(str(output_path / 'rendered_mask.png'), rendered_mask)
        self.save_img(str(output_path / 'overlay.png'), result)        
...
def main():
    ...
    # Refine pose (rotation only, translation fixed)
    result = refiner.refine_pose(
        target_mask, 
        config['init_rvec'], 
        config['init_tvec'],  # This will remain fixed
        strategy='multi_stage'
    )
    
    if result['success']:
        print("✓ 3-DOF Pose optimization successful!")
        print(f"Final rotation: {result['rvec']}")
        print(f"Fixed translation: {result['tvec']}")
        ...
        print("✗ 3-DOF Pose optimization failed")
