...
class process_scene_data:
    ...
    def mask_objects(self):
        for i, cam_row in enumerate(self.scene_data):
            ...
            # print("scene_id: ", cam_row["scene_id"], " image_index: ", cam_row["image_index"], " camera_type: ", cam_row["camera_type"])
            ...
            maskrcnn_output_path = os.path.join(self.meta_data_path, f"{cam_row['scene_id']}_{cam_row['image_index']}_{i}")
            ...
            # print(cam_row["rgb_image_path"], " detection_json: ", detection_json)
            cam_row["detection_json"] = detection_json

            self.display_results["images"].append(cam_row["rgb_img"])
            self.display_results["polygons"].append(detection_json["results"][0]["masks"])
            self.display_results["camera_ids"].append(cam_row["camera_type"])
    ...
    def consolidate_detections(self):
        ...
        for row_num, row in enumerate(self.scene_data):
            ...
            for mask_idx, mask in enumerate(detection_data['results'][0]['masks']):
                ...
                print("object ID: ", object_id, " camera ID: ", camera_id, " location_world: ", location_world, " location_camera: ", mask['object_center_3d_local'])
        ...
        print("consolidated_detections: ", self.consolidated_detections)
        ...
        for i in range(0,len(self.consolidated_detections)):
            ...
            for j in range(0, len(self.consolidated_detections[i])):
                ...
                if num_matches is not None:
                    ...
                    K = np.array([float(self.scene_data[row_num][f'k{i//3+1}{i%3+1}']) for i in range(9)]).reshape(3, 3)
                    # Extract depth scale
    ...
    def compute_6d_poses(self):
        ...
        for result in self.detections_homographies:
            ...
            if result['num_matches'] == 0:
                ...
                self.display_results["results_summary"].append({
                    "camera ID": ref_row['camera_type'],
                    "object ID": ref_row['object_id'],
                    "R_test_cam": R_test_cam,
                    "T_test_cam": T_test_cam,
                    "R_test_world": R_test_world,
                    "T_test_world": T_test_world,
                    "rmse": rmse
                })
