# config.py - Enhanced for parallel object registration
import yaml
import os
import sys
import torch
...
def print_step_timing(step_name, start_time=None, end_time=None):
...
def register_single_object(args):
    """
...
def register_all_objects_parallel(extracted_clouds_list, scene_pcl, config, max_workers=None):
    """
    ...
    registration_args = [
        (object_pcl, object_info, scene_pcl, config, idx)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        future_to_index = {
            executor.submit(register_single_object, args): idx
...
def print_registration_summary(registration_results):
...
step_start = print_step_timing("Object Detection (MaskRCNN Inference)")

maskrcnn_image_path = "/home/rama/bpc_ws/bpc/datasets/ipd_val/000000/images/000000_cam1.jpg"
...
detection_json = infer(maskrcnn_image_path, maskrcnn_model_path, maskrcnn_output_path, 
      maskrcnn_category_txt_path, maskrcnn_confidence_threshold, visualize_and_save=config['maskrcnn_visualization_and_save'])

step_end = time.time()
print_step_timing("Object Detection (MaskRCNN Inference)", step_start, step_end)

## Step 2: Build PCL
step_start = print_step_timing("Point Cloud Generation")

pcl_builder_rgb_image_path = "/home/rama/bpc_ws/bpc/ipd/val/000000/rgb_cam1/000000.png"
pcl_builder_depth_image_path = "/home/rama/bpc_ws/bpc/ipd/val/000000/depth_cam1/000000.png"
pcl_builder_camera_parameters_path = config['pcl_builder_camera_parameters_path']
pcl_builder_output_file_path = "/home/rama/bpc_ws/bpc/pcl_builder/output/" + scene_id + "_" + camera_id + "_" + img_id + ".png"

scene_pcl, width, height = build_pcl(pcl_builder_rgb_image_path, pcl_builder_depth_image_path, 
          pcl_builder_camera_parameters_path, pcl_builder_output_file_path, save_pcl=config['save_pcl'])

print("scene pcl size: ", scene_pcl)

step_end = time.time()
print_step_timing("Point Cloud Generation", step_start, step_end)

## Step 3: ROI extraction
step_start = print_step_timing("ROI Extraction")

roi_extraction_output_path = config["roi_extraction_output_path"]

extracted_clouds_list = extract_labeled_point_clouds(scene_pcl, detection_json["results"][0], roi_extraction_output_path, save_rois=config['save_rois'])

step_end = time.time()
print_step_timing("ROI Extraction", step_start, step_end)

print(f"Total objects detected and extracted: {len(extracted_clouds_list)}")

## Step 4: Parallel Registration of ALL objects
step_start = print_step_timing("Parallel Point Cloud Registration")

# Register all objects in parallel
registration_results = register_all_objects_parallel(
...
step_end = time.time()
print_step_timing("Parallel Point Cloud Registration", step_start, step_end)

# Print detailed results
print_registration_summary(registration_results)

# Save all transformations to a file for later use
transformations_file = os.path.join(config["register_output_path"], "all_transformations.yaml")
