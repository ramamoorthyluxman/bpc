...
def process_single_result_optimized(output, image_path, original_image, categories, output_dir, save_json=True, show_centers=True):
    """
    Process a single result with optimized GPU-CPU transfers
    
    Key optimization: Only transfer to CPU when needed for final saving
    """
    # Early exit for empty results (no GPU transfer needed)
    if len(output['boxes']) == 0:
        # Save empty results quickly
...
def process_batch_optimized(image_paths, model, categories, output_dir, device, threshold=0.5, save_json=True, show_centers=True):
    """
    ...
    for output, image_path, original_image in zip(outputs, valid_paths, original_images):
        detections = process_single_result_optimized(
            output, image_path, original_image, categories, output_dir, save_json, show_centers
        )
...
def infer(
    ...
    for i in range(0, len(image_paths), batch_size):
        ...
        batch_detections = process_batch_optimized(
            batch_paths, model, categories, output_path, 
            device, confidence_threshold, save_json=True, show_centers=True
        )
