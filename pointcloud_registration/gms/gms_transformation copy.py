...
def get_transformation(gms_matches, kp1, kp2):
    ...
    inliers, H = ransac(match_points, 0.5, 2000)
    return inliers, H
...
def gms_transformation(img1, img2):
    ...
    start = time.time()
    matches_gms = matchGMS(img1.shape[:2], img2.shape[:2], kp1, kp2, matches_all, withScale=False, withRotation=False, thresholdFactor=10)
    end = time.time()
...
if __name__ == '__main__':
    img2 = cv2.imread("/home/rama/bpc_ws/bpc/pointcloud_registration/output/test_images/test_04_obj_000018_polygon.png")
