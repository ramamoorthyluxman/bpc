...
def main():
    ...
    tester_args = {'network': 'host', 'extension_blacklist': {}, 
                   'operating_mode': OPERATIONS_NON_INTERACTIVE,
                   'env': [[f'BOP_PATH:/opt/ros/underlay/install/datasets']],
                   'volume': [[f"{args_dict['dataset_directory']}:/opt/ros/underlay/install/datasets/lm"]],
                   }
    ...
    zenoh_args = {'network': 'host', 'extension_blacklist': {}, 
                   'operating_mode': OPERATIONS_NON_INTERACTIVE,
                   }

    print("Buiding zenoh env")
    dig_zenoh = DockerImageGenerator(tester_extensions, tester_args, 'eclipse/zenoh:1.1.1')
    ...
    tester_thread = threading.Thread(target=run_instance, args=(dig_tester, tester_args))
    tester_thread.start()
    #TODO Redirect stdout
